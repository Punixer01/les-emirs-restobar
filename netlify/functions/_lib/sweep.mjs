...
export async function sweepExpired(force) {
  ...
  try {
    /* 1) Same day, more than 30 minutes past the booked time and still not
       arrived -> no-show. The table is deliberately LEFT ASSIGNED so the owner
       can see who is holding it and release it himself. */
    ...
    if (lateCut > 0) {
      ...
      if (noShows.length) {
        await sql`
          update reservations set status = 'no_show', updated_at = datetime('now')
          where status = 'accepted' and res_date = date('now') and arrived_at is null
            and (cast(substr(res_time,1,2) as integer) * 60
               + cast(substr(res_time,4,2) as integer)) < ${lateCut}`;
    ...
    /* 2) The day has gone by and it was never honoured -> expired. */
